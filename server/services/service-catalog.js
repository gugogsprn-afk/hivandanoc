/**
 * Service catalog — normalize categories and filter/group published services.
 */
...
function buildServiceCatalog(content, categoryId) {
  const services = content?.departments || [];
  const categories = content?.serviceCategories || [];
  const groups = groupServicesByCategory(services, categories, categoryId);
  const filtered = filterServicesByCategory(services, categoryId);
